...
export function Reviews({ initialReviews }: { initialReviews?: Review[] | null }) {
  ...
  return (
    <section id="reviews" className="brand-surface-dark relative py-20 sm:py-28">
      {/* Subtle texture */}
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {reviews.map((review, i) => (
            <Reveal key={review._id} delay={Math.min(i + 1, 4) as 0 | 1 | 2 | 3 | 4}>
              <div className="relative flex h-full flex-col justify-between rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm sm:p-7">
                <div>
